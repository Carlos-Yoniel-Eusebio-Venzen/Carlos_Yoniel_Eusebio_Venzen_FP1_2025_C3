...
/* Examen de admision.
El programa, al recibir cmo dato un archivo de acceso directo que contiene informacion sobre los alumnos que presentaron el examen de admision a una universidad, genera informacion importante para el Departamento de Control Escolar. */
...
ap = fopen("alu1.dat", "r");
/* Observa que los archivos car1.dat, car2.dat, car3.dat, car4.dat y car5.dat se abren en la modalidad para escribir y leer. */
c1 = fopen("car1.dat", "w+");
c2 = fopen("car2.dat", "w+");
c3 = fopen("car3.dat", "w+");
c4 = fopen("car4.dat", "w+");
...
/* Esta funcion genera un archivo de los alumnos admitidos en cada una de las carreras de la universidad. */
{
alumno alu;
rewind(ap);
fread(&alu, sizeof(alumno), 1, ap);
...
/* Esta funcion se utiliza para obtener el promedio que consiguieron los alumnos admitidos en cada una de las carreras. */
{
alumno alu;
float cal[5], sum;
int i, j;
i = 0;
sum = 0;
rewind(c1);     /* Es importante posicionarse al inicio del archivo, pues de lo contrario se generaria un error al ejecutar el programa. */
