...
/* Se abre el archivo para escritura. En la misma instruccion se verifica si se pudo abrir. */
{
    ...
    while (res)
    {
        ...
        if (res)
            fputs("\n", ar);
            /* Se indica un salto de linea, excepto en la ultima caena. Si no se hiciera esta indicacion, la funcion fputs pegaria las cadenas y luego tendriamos dificultades en el momento de leerlas. Por otra parte, si realizaremos este salto de linea al final de la ultima cadena, en la escritura se repetiria la ultima cadena. */
    }
    fclose(ar);
}
else
