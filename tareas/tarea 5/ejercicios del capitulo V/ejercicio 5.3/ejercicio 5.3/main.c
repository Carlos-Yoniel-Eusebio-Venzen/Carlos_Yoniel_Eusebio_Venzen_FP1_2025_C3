...
printf("\nX = %d \t Y = %d \t[0] = %d \t Z[1] = %d \t Z[3] = %d \t Z[4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);

int *IX;            /* IX representa un apuntador a un entero. */
IX = &X;            /* IX apunta a X. IX tiene la direccion de X. */
Y = *IX;            /* Y tomael valor de X, ahora vale 3. */
*IX = 1;            /* X se modifica, ahora vale 1. */
printf("\nX = %d \t Y = %d \t[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z[4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
...
*IX = *IX - 5;      /* Z[2] se vuelve a modificar, ahora vale 12. */
printf("\nX = %d \t Y = %d \t[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z[4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
...
X = *(IX+1);        /* X se modifica. El apuntador IX accede temporalmente a Z[3], por lo tanto X toma este valo (8). OBserva que IX no se reasigna */
Y = *IX;            /* Y se modifica, toma el valorf de Z[2] (12). */
printf("\nX = %d \t Y = %d \t[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z[4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
...
Y = *IX;            /* Y se modifica, toma el valor (basura) de un celda incorrecta . Es un error que no señala el compilador del lenguaje C. */
printf("\nX = %d \t Y = %d \t[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z[4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
