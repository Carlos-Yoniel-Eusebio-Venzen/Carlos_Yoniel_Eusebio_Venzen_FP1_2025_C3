...
/* UNiversidad.
El programa, al recibir informacion sobre el numero de alumnos que han ingresado a sus ocho diferentes carreras en los dos semestres lectivos de los ultimos cinco años, obtiene informacion util para el departamento de escolar. */
...
Funcion2(UNI, F, C, P);
Funcion3(UNI, 6, C, P);
/* Se coloca el 6 como parametro ya que es la clave de la Ingeneria en computacion. */
}
...
for (K = 0; K < PR; K++)
    for (I = 0; I < FI; I++)
        for (J = 0; J < CO; J++)
        {
            printf("Ano: %d\tCarrera: %d\tSemetres: %d ", K + 1, I + 1, J + 1);
            scanf("%d", &A[I][J][K]);
        }
...
/* Esta funcion se utiliza para determinar el año en el que ingreso el mayor numero de alumnos a la universidad. Observa que el primer ciclo extremo corresponde a los años. */
{
...
/* Esta funcion se utiliza para determinar la carrera que recibio el mayor numero de alumnos el ultimo año. Observa que no se utiliza un ciclo para los planos de la profundidad, ya que es un dato (PR). */
{
...
/* Esta funcion se utiliza para determinar el ano en el que la carrera ingenieria en computacion recibio el mayor numero de alumnos. Observa que no se utiliza un ciclo para trabajar con las filas, ya qie es un dato (FI). */
{
