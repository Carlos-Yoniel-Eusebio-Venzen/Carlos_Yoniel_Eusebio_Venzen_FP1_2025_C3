...
while (OPE)
{
    switch (OPE)
    {
        ...
        case 3: Nuevos_Productos(INV, &TAM);
            /* Se pasa el parametro por referencia, porque se puede modificar el numero de elementos del arrwglo en la funcion. */
            break;
...
/* Esta funcion se utiliza para leer un arreglo  unidimensional de tipo estructura producto de T elementos. */
{
int I;
for (I = 0; I < T; I++)
{
...
/* Esta funcion se utiliza para manejar las venta a un cliente. Se ingresan productos y cantidades, el fin de datos esta dado por el cero. Ademas de obtener el total de las ventas, se actualiza el inventario. */
{
int CLA, CAN, I, RES;
float TOT, PAR;
printf("\nIngresed clave del producto -0 para salir-: ");
scanf("%d", &CLA);
TOT = 0.0;
while (CLA)
{
    ...
    /* Se realiza una busqueda para localizar la clave cel producto. */
        I++;
...
/* Esta funcion se utiliza para incorporar nuevos productos al inventario. Dado que los productos se encuentran ordenados por clave, puede suceder que al insertar un nuevo producto haya que mover los elementos del arreglo para que continuen ordenados. */
{
    int  CLA, I, J;
    printf("\nIngrese clave del producto -0 para salir-: ");
    scanf("%d", &CLA);
    while ((*T < 30) && (CLA))
    {
        ...
        if (I == *T)        /* Se inserta el elemento en la ultima posicion. */
        {
            ...
            *T = *T + 1;
            ...
            if (A[I].clave == CLA)
                printf("\nEl producto ya se encuentra en el inventaio");
            else
            {
                for (J = *T; J > I; J--)
