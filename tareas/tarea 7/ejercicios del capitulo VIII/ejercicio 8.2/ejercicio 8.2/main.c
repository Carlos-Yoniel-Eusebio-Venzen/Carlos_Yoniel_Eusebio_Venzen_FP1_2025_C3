...
/* estructuras-2.
El programa muestra la manera en que se declara una estructura, asi como la forma en que se tiene a los campos de los apuntadores de tipo estructura tanto para lectura como para escritura. Se utiliza ademas una funcion que recibe como parametro un apuntador de tipo estructura. */
...
/* Nota que al apuntador a4 es necesario asignarle una direccion de memoria. Para tener acceso a los campos de un apuntador de tipo estructura, utiliza uno de los dos formatos siguietes:
...
/* Esta funcion permite leer los campos de un apuntador de tipo de tipo estructura alumno. */
{
printf("\nIngrese la matricula del alumni: ");
scanf("%d", &(*a).matricula);
fflush(stdin);
